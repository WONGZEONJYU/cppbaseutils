...
public:
    ...
    explicit XCounting_Semaphore(const std::ptrdiff_t &desired): count_(desired) {
        assert(desired >= 0 && desired <= max());
    }

    ~XCounting_Semaphore() = default;

    void release(const std::ptrdiff_t &update = 1) const{
    ...
    auto try_acquire_for(const std::chrono::duration<Rep_, Period_>& del_time) const {
        std::unique_lock lock(m_mutex_);
    ...
    auto try_acquire_until(const std::chrono::time_point<Clock_, Duration_>& abs_time_) const {
        std::unique_lock lock(m_mutex_);
