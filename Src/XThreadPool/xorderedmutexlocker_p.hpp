...
public:
    XOrderedMutexLocker(std::mutex * const m1, std::mutex * const m2)
        : m_mtx1_(m1 == m2 ? m1 : std::less<std::mutex *>()(m1, m2) ? m1 : m2),
          m_mtx2_(m1 == m2 ?  nullptr : std::less<std::mutex *>()(m1, m2) ? m2 : m1) {
        relock();
    }
    ...
    static bool relock(std::mutex * const mtx1, std::mutex * const mtx2) {
