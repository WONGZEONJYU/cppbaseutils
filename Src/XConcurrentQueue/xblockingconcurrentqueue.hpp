...
public:
	...
	XBlockingConcurrentQueue(XBlockingConcurrentQueue && other) noexcept = default;
	XBlockingConcurrentQueue& operator=(XBlockingConcurrentQueue && other) noexcept = default;
	...
	// Thread-safe.
	constexpr bool enqueue(T const & item) {
	...
	// Thread-safe.
	constexpr bool enqueue(Base::producer_token_t const & token, T const & item) {
	...
	// is 0).
	// Thread-safe.
	constexpr bool try_enqueue(T const & item){
	...
	// Thread-safe.
	constexpr bool try_enqueue(T && item) {
	...
	// Thread-safe.
	constexpr bool try_enqueue(Base::producer_token_t const & token, T const & item) {
	...
	// Does not allocate memory. Fails if not enough room to enqueue.
	// Thread-safe.
	constexpr bool try_enqueue(Base::producer_token_t const & token, T && item) {
	...
	// Never allocates. Thread-safe.
	template<typename U>
	constexpr void wait_dequeue(Base::consumer_token_t & token, U & item)
	...
	// Never allocates. Thread-safe.
	template<typename U>
	constexpr bool wait_dequeue_timed(Base::consumer_token_t & token, U & item, std::int64_t const timeout_usecs) {
		if (!this->m_sema_->wait(timeout_usecs)) { return {}; }
	...
	// Never allocates. Thread-safe.
	template<typename U, typename Rep, typename Period>
	constexpr bool wait_dequeue_timed(Base::consumer_token_t & token, U & item, std::chrono::duration<Rep, Period> const& timeout)
