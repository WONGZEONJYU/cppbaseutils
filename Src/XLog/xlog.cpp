...
void XLog::setLogFileConfig(std::string_view const & base_name, 
    ...
    d->m_max_file_size_.storeRelaxed(max_size_mb * 1024 * 1024);
...
void XLog::cleanupOldLogFiles() const noexcept {
    ...
    try {
        ...
        if (!exists(d->m_log_directory_)) { return; }

        auto const retention_days{d->m_retention_days_.loadRelaxed()};
        if (retention_days <= 0) { return; } // 不限制保存天数
...
{ d_func()->m_color_output_.storeRelaxed(enable); }

void XLog::setAsyncQueueSize(std::size_t const size) noexcept
{ d_func()->m_max_queue_size_.storeRelaxed(size); }

void XLog::enableCrashDiagnostics(bool const enable) {
    d_func()->m_crash_diagnostics_.storeRelaxed(enable);
...
[[maybe_unused]] bool XLog::waitForCompletion(std::chrono::milliseconds const & timeout) {
    ...
    if ( std::chrono::milliseconds::zero() == timeout ) {
        d->m_queue_cv_.wait(lock, pred);
        return true;
    }
    return d->m_queue_cv_.wait_for(lock, timeout,pred);
}
...
std::string XLog::getCurrentTimestamp() {
    ...
    public:
        explicit SafeTimeFormatter(std::time_t const & time) {
            ...
            if (!valid_) { valid_ = !gmtime_s(&tm_buffer_, &time); } // Fallback to UTC
#else
            // POSIX系统：使用线程安全版本
            valid_ = localtime_r(&time, &tm_buffer_) != nullptr;
            if (!valid_) { valid_ = gmtime_r(&time, &tm_buffer_) != nullptr; } // Fallback to UTC
#endif
        }
        // 禁用拷贝和移动，确保线程安全
        ...
        [[nodiscard]] const std::tm * get() const noexcept { return valid_ ? &tm_buffer_ : nullptr;  }
    };
    ...
    if (!formatter.is_valid()) { return {"1970-01-01 00:00:00.000"}; }
...
std::string XLogPrivate::findLatestLogFile() const {
    try
    {
        if (!std::filesystem::exists(m_log_directory_)) { return {}; }
...
void XLogPrivate::processLogQueue(){
    ...
    while (m_running_.loadAcquire() || !m_log_queue_.empty()) {
        ...
        m_queue_cv_.wait(lock, [this]{
            return !m_log_queue_.empty() || m_shutdown_requested_.loadAcquire();
        });
        ...
        while (!m_log_queue_.empty()) {
            ...
            if ((output & LogOutput::CONSOLE) != LogOutput{}) { writeToConsole(msg); }

            if ((output & LogOutput::FILE) != LogOutput{}) { writeToFile(msg); }
        ...
        if (m_shutdown_requested_.loadAcquire()) { break; }
    }
}
...
void XLogPrivate::writeToFile(LogMessage const & msg) {
    ...
    if (!m_file_stream_ || !m_file_stream_->is_open()) {
        ...
        try {
            ...
            { m_current_file_size_.storeRelaxed(std::filesystem::file_size(m_current_log_file_)); }
    ...
    *m_file_stream_ << formatted << '\n';
    ...
    // 更新文件大小
    m_current_file_size_.fetchAndAddRelaxed(formatted.length() + 1);
}
...
bool XLogPrivate::shouldRotateFile() const noexcept {
    auto const max_size{m_max_file_size_.loadRelaxed()};
    return max_size > 0 && m_current_file_size_.loadRelaxed() >= max_size;
}
...
#else
    auto const handleCrash_{ [](int const sig,siginfo_t*,void*){ handleCrash(sig); } };
    SignalRegister(SIGSEGV,0,handleCrash_);
    SignalRegister(SIGABRT,0, handleCrash_);
    SignalRegister(SIGFPE,0,handleCrash_);
    SignalRegister(SIGILL,0,handleCrash_);
    SignalRegister(SIGTERM,0,handleCrash_);
#endif
}
...
#else
    SignalUnRegister(SIGSEGV);
    SignalUnRegister(SIGABRT);
    SignalUnRegister(SIGFPE);
    SignalUnRegister(SIGILL);
    SignalUnRegister(SIGTERM);
#endif
}
...
void XLogPrivate::writeCrashLog(std::string_view const & crash_info) {
    try {
        ...
        if (std::ofstream crash_file(filename_oss.str()) ; crash_file.is_open())
...
void XLogPrivate::initializeLogFile() {
    try {
            ...
            ; !latest_file.empty())
        {
            ...
            if (auto const max_size{m_max_file_size_.loadRelaxed()}
                ; !max_size || file_size < max_size)
            {
                // 文件未满，继续使用
                m_current_log_file_ = latest_file;
                m_log_file_path_ = latest_file;
                m_current_file_size_.storeRelaxed(file_size);
                return;
            }
        }
