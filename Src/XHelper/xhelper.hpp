...
template <typename T> X_API inline T *xGetPtrHelper(T *ptr) noexcept { return ptr; }
template <typename Ptr> X_API inline auto xGetPtrHelper(Ptr &ptr) noexcept -> decltype(ptr.get())
{ static_assert(noexcept(ptr.get()), "Smart d pointers for X_DECLARE_PRIVATE must have noexcept get()"); return ptr.get(); }
template <typename Ptr> X_API inline auto xGetPtrHelper(Ptr const &ptr) noexcept -> decltype(ptr.get())
{ static_assert(noexcept(ptr.get()), "Smart d pointers for X_DECLARE_PRIVATE must have noexcept get()"); return ptr.get(); }
...
[[maybe_unused]] [[nodiscard]] X_API inline std::unique_ptr<T> make_Unique(Args && ...args) noexcept {
    try{
 ...
 */
template<typename T, typename ... Args>
[[maybe_unused]] [[nodiscard]] X_API inline std::shared_ptr<T> make_Shared(Args && ...args) noexcept {
    try{
...
[[maybe_unused]] X_API inline void for_each_tuple(Tuple && tuple_, Pred && pred_) {
...
namespace TuplePrivate {
    ...
    X_API inline auto Left_Tuple_n_(const std::tuple<Args...> & tuple_) {
    ...
    // 方法1: 获取后N个元素的通用模板
    template<const std::size_t N, typename... Args>
    X_API inline auto Last_Tuple_n_(const std::tuple<Args...> & tuple_) {
        static_assert(N <= sizeof...(Args), "N must be <= tuple size");
        static_assert(N <= std::tuple_size_v<std::decay_t<decltype(tuple_)>>,"N must be <= tuple size");
    ...
    X_API inline auto skip_front_n_(const std::tuple<Args...>& tuple_) {
...
[[maybe_unused]] X_API inline auto Left_Tuple(const Tuple & tuple_) {
 ...
 */
template<const std::size_t N,typename Tuple>
[[maybe_unused]] X_API inline auto Last_Tuple(const Tuple & tuple_) {
 ...
 */
template<const std::size_t N,typename Tuple>
[[maybe_unused]] X_API inline auto SkipFront_Tuple(const Tuple &tuple_) {
